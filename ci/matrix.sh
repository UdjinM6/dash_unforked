...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export BITCOIN_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports --enable-crash-hooks --enable-debug"
  export CPPFLAGS="-DENABLE_DASH_DEBUG -DARENA_DEBUG"
  export PYZMQ=true
  export RUN_UNITTESTS=true
  export RUN_INTEGRATIONTESTS=true
  ...
  export HOST=x86_64-unknown-linux-gnu
  export DEP_OPTS="NO_UPNP=1 DEBUG=1"
  export BITCOIN_CONFIG="--enable-zmq --enable-glibc-back-compat --enable-reduce-exports --enable-crash-hooks --enable-debug --enable-c++17"
  export CPPFLAGS="-DENABLE_DASH_DEBUG -DARENA_DEBUG"
  export PYZMQ=true
  export RUN_UNITTESTS=true
  export RUN_INTEGRATIONTESTS=true
