...
bool AppInitMain(const CoreContext& context, NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (strSocketEventsMode == "select") {
        connOptions.socketEventsMode = CConnman::SocketEventsMode::SELECT;
#ifdef USE_POLL
    } else if (strSocketEventsMode == "poll") {
        connOptions.socketEventsMode = CConnman::SocketEventsMode::POLL;
#endif
#ifdef USE_EPOLL
    } else if (strSocketEventsMode == "epoll") {
        connOptions.socketEventsMode = CConnman::SocketEventsMode::EPOLL;
#endif
#ifdef USE_KQUEUE
    } else if (strSocketEventsMode == "kqueue") {
        connOptions.socketEventsMode = CConnman::SocketEventsMode::KQUEUE;
#endif
    } else {
