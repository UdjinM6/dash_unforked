...
CAmount CWallet::GetBalance(const isminefilter& filter, const int min_depth, const bool fAddLocked) const
{
    CAmount nTotal = 0;
    {
        ...
        for (auto pcoin : GetSpendableTXs()) {
            if (pcoin->IsTrusted(*locked_chain) && ((pcoin->GetDepthInMainChain(*locked_chain) >= min_depth) || (fAddLocked && pcoin->IsLockedByInstantSend()))) {
...
CAmount CWallet::GetAnonymizedBalance(const CCoinControl* coinControl) const
{
    ...
    for (auto pcoin : GetSpendableTXs()) {
        nTotal += pcoin->GetAnonymizedCredit(*locked_chain, coinControl);
    }
...
CAmount CWallet::GetUnconfirmedBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        LOCK(cs_wallet);
        for (auto pcoin : GetSpendableTXs()) {
            if (!pcoin->IsTrusted(*locked_chain) && pcoin->GetDepthInMainChain(*locked_chain) == 0 && !pcoin->IsLockedByInstantSend() && pcoin->InMempool())
...
bool CWalletTx::AcceptToMemoryPool(interfaces::Chain::Lock& locked_chain, const CAmount& nAbsurdFee, CValidationState& state) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
