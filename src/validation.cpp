...
CAmount GetBlockSubsidy(const CBlockIndex* const pindex, const Consensus::Params& consensusParams)
{
    ...
    return GetBlockSubsidyInner(pindex->pprev->nBits, pindex->pprev->nHeight, consensusParams, isMNRewardReallocated, /*fSuperblockPartOnly=*/ false);
}
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!MasternodePayments::IsBlockPayeeValid(*sporkManager, *governance, *block.vtx[0], pindex->nHeight, blockSubsidy, feeReward)) {
        // NOTE: Do not punish, the node might be missing governance data
