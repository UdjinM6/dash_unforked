                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (conflictLock) {
        ...
        return state.Invalid(ValidationInvalidReason::TX_CONFLICT, error("AcceptToMemoryPool : Transaction %s conflicts with locked TX %s", hash.ToString(), conflictLock->txid.ToString()), REJECT_INVALID, "tx-txlock-conflict");
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = llmq::quorumInstantSendManager->GetConflictingLock(*tx)) {
                if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    ...
                    return state.Invalid(ValidationInvalidReason::TX_CONFLICT, error("ConnectBlock(DASH): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()), REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    }
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.Invalid(ValidationInvalidReason::CONSENSUS, error("ConnectBlock(DASH): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        return state.Invalid(ValidationInvalidReason::CONSENSUS, error("ConnectBlock(DASH): couldn't find masternode or superblock payments"), REJECT_INVALID, "bad-cb-payee");
    }
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        if (miSelf != m_block_index.end()) {
            ...
            if (pindex->nStatus & BLOCK_CONFLICT_CHAINLOCK)
                return state.Invalid(ValidationInvalidReason::CACHED_INVALID, error("%s: block %s is marked conflicting", __func__, hash.ToString()), 0, "duplicate");
            return true;
        }
        ...
        if (llmq::chainLocksHandler->HasConflictingChainLock(pindexPrev->nHeight + 1, hash)) {
            ...
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_PREV, error("%s: header %s conflicts with chainlock", __func__, hash.ToString()), REJECT_INVALID, "bad-chainlock");
        }
    }
    if (pindex == nullptr)
