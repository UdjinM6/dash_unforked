...
Wallets with 100k+ txes should now be able to rescan and not hang forever
processing notifications for every tx. Running `keypoolrefill` with large number
of keys and will no longer lockup GUI and can be interrupted. Running
`upgradetohd` can also be interrupted now.
...
- BLS library was updated to version 1.3.0 to fix a cople of tiny issues
